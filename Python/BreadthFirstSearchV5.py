...
def bfs_edges(G, visited, company, source, reverse=False, depth_limit=None):
    ...
    for e in generic_bfs_edges(G, visited, company, source, successors, depth_limit):
        yield e
G = nx.read_edgelist('CFRPMLinks.csv', create_using=nx.DiGraph(), delimiter=',',nodetype=int, data=(('FacType',int),('LinkRev',int),('COUNT_DAIL',int),('COUNT_AM',int),('COUNT_MD',int),('COUNT_PM',int),('COUNT_NT',int),))
