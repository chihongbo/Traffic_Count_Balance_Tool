...
VARIABLES
CAPFACPARA(T) is the Capacity factor parameter d;
CAPFACPARA.lo(T)=0.7;
CAPFACPARA.L(T) = 1.0;
...
*OBJF.. OBJ =E=SUM((J,T)$(OBSSEGTT(J,T) gt 0),sqr(sum(I$linkseg(I,J),(FF(I)*(1+VC(I,T)*CAPFACPARA(J,T)*(BPRPARA(J,"1","1")$(WEAVE(I)=0)+BPRPARA(J,"2","1")$(WEAVE(I)>0))+(BPRPARA(J,"1","2")$(WEAVE(I)=0)+BPRPARA(J,"2","2")$(WEAVE(I)>0))*(VC(I,T)*CAPFACPARA(J,T))**(BPRPARA(J,"1","3")$(WEAVE(I)=0)+BPRPARA(J,"2","3")$(WEAVE(I)>0)))- OBSTT(I,T)))/OBSSEGTT(J,T)))+
*0.1*sum((J,M,N)$(ord(N) = card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)))+0.1*sum((J,M,N)$(ord(N) <> card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)))+0.5*sum((J,T),sqr((CAPFACPARA(J,T)-1.0)));

OBJF.. OBJ =E=SUM((J,T)$(OBSSEGTT(J,T) gt 0),sqr(sum(I$linkseg(I,J),(FF(I)*(1+VC(I,T)*CAPFACPARA(T)*(BPRPARA(J,"1","1")$(WEAVE(I)=0)+BPRPARA(J,"2","1")$(WEAVE(I)>0))+(BPRPARA(J,"1","2")$(WEAVE(I)=0)+BPRPARA(J,"2","2")$(WEAVE(I)>0))*(VC(I,T)*CAPFACPARA(T))**(BPRPARA(J,"1","3")$(WEAVE(I)=0)+BPRPARA(J,"2","3")$(WEAVE(I)>0)))- OBSTT(I,T)))/OBSSEGTT(J,T)))+
0.1*sum((J,M,N)$(ord(N) = card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)))+0.1*sum((J,M,N)$(ord(N) <> card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)))+0.1*sum((T),sqr((CAPFACPARA(T)-1.0)));
MODEL ODE /ALL/;
...
put estdemand1;
loop((T), put T.tl:>4:1; put CAPFACPARA.L(T):>12:5; put /;);
