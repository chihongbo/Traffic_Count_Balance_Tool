...
OBJF.. OBJ =E=SUM((J,T)$(OBSSEGTT(J,T) gt 0),sqr(sum(I$linkseg(I,J),(FF(I)*(1+VC(I,T)*(BPRPARA(J,"1","1")$(WEAVE(I)=0)+BPRPARA(J,"2","1")$(WEAVE(I)>0))+(BPRPARA(J,"1","2")$(WEAVE(I)=0)+BPRPARA(J,"2","2")$(WEAVE(I)>0))*VC(I,T)**(BPRPARA(J,"1","3")$(WEAVE(I)=0)+BPRPARA(J,"2","3")$(WEAVE(I)>0)))- OBSTT(I,T)))/OBSSEGTT(J,T)))+0.01*sum((J,M,N)$(ord(N) = card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)))+0.001*sum((J,M,N)$(ord(N) <> card(N)),sqr((BPRPARA(J,M,N)-BPR(J,M,N))/BPR(J,M,N)));
MODEL ODE /ALL/;
